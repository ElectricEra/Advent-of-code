...
parsedData[startingPoint[0]][startingPoint[1]] = 'a';
parsedData[endPoint[0]][endPoint[1]] = 'z';
parsedData[startingPoint[0]][startingPoint[1]] = {
  value: parsedData[startingPoint[0]][startingPoint[1]],
  weight: 0
}
...
const results = [];
parsedData.forEach((row, i) => row.forEach((el, j) => {
  if (el === 'a') results.push(findPath([i, j], endPoint, alphabet));
}))
const result = results.filter(Number).sort()[0]
